...
/// Evaluates MIME compatibility using category-level matching and legacy aliases.
...
/// Checks for sensitive content hints to prevent unauthorized data persistence.
...
/// Configures a pipe for blocking I/O and expands capacity for binary payloads.
fn make_pipe(is_image: bool) -> Option<(std::fs::File, OwnedFd)> {
    ...
    unsafe {
        ...
        Some((std::fs::File::from_raw_fd(fds[0]), OwnedFd::from_raw_fd(fds[1])))
    }
}
    ...
    fn event(_: &mut Self, _: &ExtDataControlOfferV1, ev: ext_data_control_offer_v1::Event, data: &OfferData, _: &Connection, _: &QueueHandle<Self>) {
        if let ext_data_control_offer_v1::Event::Offer { mime_type } = ev {
            if let Ok(mut mimes) = data.mimes.lock() {
                if !mimes.contains(&mime_type) { mimes.push(mime_type); }
            }
        }
    }
}
...
impl Dispatch<ExtDataControlDeviceV1, ()> for WaylandState {
    fn event(state: &mut Self, _: &ExtDataControlDeviceV1, ev: ext_data_control_device_v1::Event, _: &(), conn: &Connection, _: &QueueHandle<Self>) {
        if let ext_data_control_device_v1::Event::Selection { id } = ev {
            ...
            // Handle provider flag reset to allow subsequent external copies
            ...
            let priority: &[&str] = &["image/webp", "image/png", "image/jpeg", "image/gif", MIME_URI_LIST, "text/plain;charset=utf-8", "text/plain"];
            ...
            if let Some(db_path) = state.db.as_ref().map(|db| db.path.clone()) {
                ...
                std::thread::spawn(move || {
                    ...
                    // Aligned buffer for optimized kernel-to-user data transfer
                    ...
                    if final_mime == MIME_URI_LIST {
                        ...
                        if let Some(line) = uri_content.lines().next() {
                            if line.starts_with("file://") {
                                ...
                                if let Ok(decoded) = percent_encoding::percent_decode_str(path_raw).decode_utf8() {
                                    ...
                                    if path.exists() && path.is_file() {
                                        ...
                                        let m = match ext.to_lowercase().as_str() {
                                            ...
                                            "webp" => Some("image/webp"), "gif" => Some("image/gif"), _ => None,
                                        };
                                        if let Some(mime) = m {
                                            if let Ok(data) = std::fs::read(path) {
                                                payload = data;
                                                final_mime = mime.to_string();
                    ...
                    if let Ok(db_conn) = rusqlite::Connection::open(&db_path) {
                        ...
                        let hash = format!("{:x}", hash_context.compute());
                        let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_millis() as i64;
                        let existing: Option<i64> = db_conn.query_row("SELECT id FROM clipboard WHERE hash = ?1 LIMIT 1", rusqlite::params![hash], |r| r.get(0)).ok();
                        ...
                        if let Some(id) = existing {
                            ...
                            let res = db_conn.execute("INSERT INTO clipboard (timestamp, mime, size, preview, content, hash) VALUES (?1, ?2, ?3, ?4, ?5, ?6)", 
                                rusqlite::params![ts, final_mime, payload.len() as i64, preview, payload, hash]);
                            if res.is_ok() && is_verbose { println!("{}", log_save(&final_mime, payload.len())); }
                        }
            ...
            ext_data_control_source_v1::Event::Send { mime_type, fd } => {
                if mime_is_compatible(&mime_type, &state.target_mime) {
                    unsafe {
                        ...
                        if flags >= 0 { libc::fcntl(raw, libc::F_SETFL, flags & !libc::O_NONBLOCK); }
                    }
                    let mut file = std::fs::File::from(fd);
                    let data = state.tx_buf.clone();
                    std::thread::spawn(move || {
